...
// Garante que somente administradores acessam este endpoint
    ...
    if ($senha !== '') {
        ...
        $stmt = $pdo->prepare(
            "UPDATE Usuario SET NOME=:nome, EMAIL=:email, TELEFONE=:tel, ENDERECO=:end, SENHA=:senha
             WHERE IDUSUARIO=:idusuario AND IDPERMISSAO=(SELECT IDPERMISSAO FROM Permissao WHERE NOME='administrador' LIMIT 1)"
        );
        $stmt->execute([
            ':nome'      => $nome,
            ':email'     => $email,
